...
run() {
  ...
  then
    "$SPARK_DIR/bin/spark-submit" --master spark://localhost:7077 --class spark.benchmark.KafkaRedisAdvertisingStream /root/spark-benchmarks/spark-benchmarks.jar "$CONF_FILE" &
    sleep 5
